...
trait HasEloquentFilter
{
	...
	{
	    $this->wheres = collect();
		$wheres       = $this->queryParams->filter(function ($value, $key) {
			return !preg_match('/\_/', $key);
		});
		foreach ($wheres as $key => $value) {
            $field = explode(':', $key);
            $maps  = $this->getMaps();
            if (count($field) > 1 and array_key_exists($field[0], $maps)) {
                $field[0] = $maps[$field[0]];
            }
            $this->wheres["{$field[0]}:{$field[1]}"] = $value;
        }
	}
	...
	private function formatCriteriaValue(array $pieces, $value)
	{
		...
		if ($filterOperator) {
			if (preg_match('/ilike/', $filterOperator)) {
				$value = '%' . $value . '%';
				return $value;
			}
		}
	...
	private function buildQueryOrderBy($model)
	{
		if ($orderBy = Arr::get($this->queryParams, '_orderBy')) {
			...
			$maps          = $this->getMaps();
			if (array_key_exists($columnName, $this->getMaps())) {
